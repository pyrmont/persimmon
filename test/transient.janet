...
# A transient divides in the operator position as its persistent collection
# does, and a spent one raises there as it does everywhere else.
(deftest calling-a-transient
  (def vector-trans (persimmon/transient (persimmon/vec :foo)))
  (is (= :foo (vector-trans 0)))
  (is (= :foo (vector-trans -1)))
  (is (thrown? (vector-trans 1)))
