...
internal representation as they can. Persimmon's vector is a bit-partitioned
trie of the kind Clojure popularised: 32 items to a node, with a tail buffer
so that repeated appends usually touch nothing but the last node.

## Structure

The library is in two layers.

`src/persimmon.c` is the core and knows nothing about any host language. Its
elements are opaque blobs of a fixed size, stored inline in the trie, and it
delegates their lifecycle to a table of callbacks the host supplies:
...
Node reference counts are atomic wherever the toolchain provides atomics,
whether through C11 `<stdatomic.h>`, the GCC and Clang builtins, or the
Windows interlocked functions. `persimm_has_atomic_refcounts` reports whether
that was the case for a given build. Where it returns false, a graph of
vectors must not be shared across threads.
...
| `(persimmon/conj vec x)`     | a vector with `x` appended                     |
| `(persimmon/assoc vec i x)`  | a vector with index `i` replaced by `x`        |
| `(persimmon/to-array vec)`   | the elements as a mutable array                |

Vectors support `length`, `get` (including negative indices), `next` and so
the whole of Janet's iteration machinery, `string`, and `hash`.
